...
document.writeln('<iframe id=endDateLayer frameborder=0 width=163 height=211 style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='<span id=tmpSelectHourLayer  style="z-index: 9999;position: absolute;top: 28; left: 30px;display: none"></span>';
strFrame+='<span id=tmpSelectMinuteLayer style="z-index:9999;position:absolute;top: 28; left: 68px;display: none"></span>';
strFrame+='<span id=tmpSelectSecondLayer style="z-index:9999;position:absolute;top: 28; left: 114px;display: none"></span>';
...
strFrame+='     <tr ><td width=142 >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=80  style="cursor:hand" title="点击这里启用/禁用时间"';
...
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间"></span>  <span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="关闭日历">关闭 </span></td></tr>';
strFrame+='    </table></td></tr>';
strFrame+='</table></div>';
...
function meizzWriteHead(yy,mm,ss)    //往 head 中写入当前的年与月
{
  ...
  ///去掉不显示秒  odatelayer.meizzSecondHead.innerText=bUseTime?(meizzTheSecond+" 秒"):"";
}
...
//        ii = i* 5;
//    
//        if (i == m) { selectInnerHTML += "<option value='"+ ii +"' selected>"+ii+"</option>\r\n"; }
//        else { selectInnerHTML += "<option value='"+ii+"'>"+ii+"</option>\r\n"; }
//    }
//    selectInnerHTML += "</select>";
...
function evaSetTime()        //设置用户选择的小时、分钟
{
  ...
  ////去掉  odatelayer.meizzSecondHead.innerText=meizzTheSecond+" 秒";
...
function evaSetTimeNow()    //设置时间控件为当前时间
{
   ...
   // odatelayer.meizzMinuteHead.innerText=new Date().getMinutes()+" 分";
    ////去掉  odatelayer.meizzSecondHead.innerText=new Date().getSeconds()+" 秒";
...
{
if (outObject.id=="start_time") 
{ 
    if (bUseTime)
    {
        outObject.value= yy + "年" + format(mm) + "月" + format(n) + "日" ;// + ":" + format(se); //注：在这里你可以输出改成你想要的格式
    }
    else
    {
...
else if(outObject.id=="stop_time") 
{
    if (bUseTime)
    {
        outObject.value= yy + "年" + format(mm) + "月" + format(n) + "日" ; // + ":" + format(se); //注：在这里你可以输出改成你想要的格式
